...
def create_exogenous_features(original_dataset, optimally_differenced_dataset, dataset_start_time, dataset_end_time, include_fractional_hour = False, ExogenousFeatureType='AdditiveandMultiplicativeExogenousFeatures', sampling_frequency='h'):
    if ExogenousFeatureType == 'NoExogenousFeatures':
    ...
    if sampling_frequency_offset <= hour_test and dataset_offset >= to_offset('3d') :
        ...
        # if ExogenousFeatureType != 'AdditiveandMultiplicativeExogenousFeatures':
        ...
        # if (ExogenousFeatureType != 'AdditiveandMultiplicativeExogenousFeatures' or include_fractional_hour == True):
        #     print(" right part true")
        #temp
        if sampling_frequency_offset < hour_test and ((ExogenousFeatureType in ['ExogenousFeaturesBasedonSeasonalityTest', 'ExogenousFeaturesBasedonSeasonalityTestWithAdditivenMultiplicative']) or include_fractional_hour == True):
    ...
    week_seasonal = None
    if ExogenousFeatureType in ['ExogenousFeaturesBasedonSeasonalityTest', 'ExogenousFeaturesBasedonSeasonalityTestWithAdditivenMultiplicative']:
    ...
    if ExogenousFeatureType in ['AdditiveandMultiplicativeExogenousFeatures', 'ExogenousFeaturesBasedonSeasonalityTestWithAdditivenMultiplicative'] and num_columns > 0:
...
def DetermineFeatureSet(Dataset, Data_train, End_validation, End_train, Dataset_start_time, Dataset_end_time, DatasetWithFeatures, weight_para=False, metric="mean_absolute_scaled_error", modeltype="tree_or_forest",InitialLags=None, ExogenousFeatureType=None, FeatureSetReduction=False, FeatureSetReductionMethod=None, FeatureSetReductionStep=5, FeatureSetReductionSubSample=0.5, RFECV_CV=2, RFECV_min_features_to_select=10, RFE_n_features_to_select=None, bayesian_trial=None, random_state_hyper=123, LastObservedDateTime=None, Frequency='h', BacktestSteps=24, PredictionSteps=24):
    ...
    if data_diff.empty:
        ...
        exog_features, calendar_features , df_exogenous_features, hour_seasonality, dow_seasonality, week_seasonality = create_exogenous_features(original_dataset=Dataset, 
                                                                                             ...
                                                                                             ExogenousFeatureType=ExogenousFeatureType, 
    ...
    if FeatureSetReduction == True:
        ...
        model_search = GeneralizedHyperparameterSearch(forecaster=hyper_forecaster, y=Dataset.loc[:End_validation, value], lags=InitialLags, steps=BacktestSteps, initial_train_size=len(Data_train), metric=metric)
        print("outside here maybe")
        ...
        # lightgbm_hyper_forecaster = create_forecaster('lightgbm', random_state=123, verbose=-1, lags=InitialLags, differentiation=differentiation, custom_params=lightgbm_best_params, weight=weight )
        # lightgbm_hyper_forecaster = create_forecaster('lightgbm', random_state=123, verbose=-1, lags=InitialLags, differentiation=differentiation, custom_params=lightgbm_best_params )
        
        if modeltype == "tree_or_forest":
            feature_forecaster = create_forecaster('autoreg_lightgbm', random_state=123, verbose=-1, lags=InitialLags, differentiation=differentiation, custom_params=best_params, weight=weight, if_exog=if_exog )

        print("outside here maybe")
        if FeatureSetReductionMethod=='RFECV':
            ...
            selected_lags, selected_exog = perform_RFECV_feature_selection(
                ...
                exog=DatasetWithFeatures.loc[:End_train, exog_features], # replace exog_features if needed
                end_train=End_train,
        ...
        selected_lags = InitialLags
        selected_exog = exog_features


    if ExogenousFeatureType != 'NoExogenousFeatures' and len(selected_exog) > 0:
        # make sure this works for the case where the below doesn't break for the noexog features
        _ , exog_timewindow = generate_exog_data( LastObservedDateTime, Frequency, PredictionSteps, '%Y-%m-%d %H:%M:%S')
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        _ , forecast_calendar_features, _, _, _, _  = create_exogenous_features(original_dataset=exog_timewindow, 
                                                                      ...
                                                                      dataset_end_time=Dataset_end_time, 
                                                                      include_fractional_hour=True, 
                                                                      ExogenousFeatureType='AdditiveandMultiplicativeExogenousFeatures', 
                                                                      sampling_frequency=Frequency)
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    if missing_values:
        DatasetSelectedFeatures = Dataset[['value', 'value_imputed']].merge(
        ...
        DatasetSelectedFeatures = Dataset[['value']].merge(
            df_exogenous_features,
            left_index=True,
            right_index=True,
            how='left'
    ...
    return selected_lags, selected_exog, differentiation, DatasetSelectedFeatures, forecast_calendar_features, hour_seasonality, dow_seasonality, week_seasonality, missing_values, weight
...
def process_data(filename, sampling_frequency=None, col_names=None, training_percentage=80, validation_percentage=10, 
    ...
    if dataset_duration >= pd.Timedelta(days=19) and len(dataset) >= 25:  
        ...
        # quick_start : linear_regression with no_exog, FeatureSetReduction = False
        if quick_start:
        ...
        if dataset_duration >= pd.Timedelta(days=3) and len(dataset) >= 72:
            ...
            # quick_start : baseline with no_exog, FeatureSetReduction = False
...
def ModelCreateTrainTestAndPredict(
    model_name,
    dataset,
    dataset_train,
    End_validation,
    ...
    y = DatasetSelectedFeatures[value].copy()
    ...
    if model_name.lower() == "baseline":
        ...
        baseline_forecaster.fit(y=DatasetSelectedFeatures.loc[:End_validation, value])
        print(DatasetSelectedFeatures.loc[:End_validation, value].tail())
        ...
        # Fit the forecaster on the entire dataset
        baseline_forecaster.fit(y=DatasetSelectedFeatures[value])
        # Make predictions
        print(DatasetSelectedFeatures[value].tail())
        ...
        return {
            ...
            'selected_lags': None,
            'selected_exog': None,
            'DatasetSelectedFeatures': DatasetSelectedFeatures,
    ...
    #add the linear regressors here
        ...
        arima_forecaster = create_forecaster(
            model_type='arima',
            y=DatasetSelectedFeatures.loc[:End_validation, value],
        ...
        arima_forecaster.fit(y=DatasetSelectedFeatures.loc[:End_validation, value], suppress_warnings=True)

        backtest_predictions = arima_forecaster.predict_interval(steps=backtest_steps, interval=interval)
        backtest_error = calculate_error(y_test, backtest_predictions[:len(y_test)], y_train, error_type=metric)

        # Calculate interval coverage
        coverage, _ = calculate_interval_coverage(DatasetSelectedFeatures, backtest_predictions, End_validation, End_test, value)

        arima_forecaster.fit(y=DatasetSelectedFeatures[value], suppress_warnings=True)
        ...
        if model_name.lower() == 'skt_lstm_deeplearning':
            ...
            backtest_prediction.index = backtest_prediction.index + time_delta
            coverage, _ = calculate_interval_coverage(DatasetSelectedFeatures, backtest_prediction, End_validation, End_test, value)
            forecaster.fit(y)
            y_pred_future = forecaster.predict(list(range(1, forecasting_steps + 1)))
        ...
        if model_name.lower() != 'direct_linearregression':
            ...
            if Select_hyperparameters:
                forecaster_search = GeneralizedHyperparameterSearch(
                    ...
                    y=DatasetSelectedFeatures.loc[:End_validation, value],
                    lags=selected_lags,
                    exog=DatasetSelectedFeatures.loc[:End_validation, selected_exog],
                    steps=forecasting_steps,
        ...
        if len(selected_exog) == 0: 
            ...
            backtest_predictions = final_forecaster.predict_interval(steps=backtest_steps, exog=DatasetSelectedFeatures.loc[DatasetSelectedFeatures.index > End_validation, selected_exog], interval=interval)
        ...
        if len(selected_exog) == 0: 
            final_forecaster.fit(
                y=DatasetSelectedFeatures[value]
            )
        else:
            final_forecaster.fit(
                y=DatasetSelectedFeatures[value],
                exog=DatasetSelectedFeatures.loc[:End_test, selected_exog]
            )
...
def Demonstration(filename, 
                  ...
                  list_of_models, 
                  interval=[10,90],
                  FeatureSetReduction=False, 
                  ExogenousFeatureType='ExogenousFeaturesBasedonSeasonalityTestWithAdditivenMultiplicative', 
                  FeatureSetReductionMethod='RFECV', 
    ...
    end_times, dataset, data_subsets, dataset_withfeatures, dataset_withfeatures_subsets, start_date_time, last_date_time, sf, lags, backtest_steps, forecasting_steps, if_use_weight, baseline_time, baseline_offsets, baseline_n_offsets, if_small_dataset, allowed_models = process_data(
        filename,
        col_names=col_names,
    ...
    if random_model_present and not quick_start_present:
        ...
        FeatureSetReduction = random.choice([True, False])
        ExogenousFeatureType = random.choice(["NoExogenousFeatures", "Additive", "AdditiveandMultiplicativeExogenousFeatures", "ExogenousFeaturesBasedonSeasonalityTestWithAdditivenMultiplicative"])
        FeatureSetReductionMethod = random.choice(["RFECV", "RFE"])
        ...
        print(f"FeatureSetReduction: {FeatureSetReduction}")
        print(f"ExogenousFeatureType: {ExogenousFeatureType}")
        print(f"FeatureSetReductionMethod: {FeatureSetReductionMethod}")
        print(f"random_state_hyper: {random_state_hyper}")

    if quick_start_present:
        FeatureSetReduction = False
        ExogenousFeatureType = "NoExogenousFeatures"
    ...
    try:
        sel_lags, sel_exogs, differentiation, dataset_feature_selection, f_c_f, hr_s, dow_s, w_s,  missing, weight = DetermineFeatureSet(
            Dataset=dataset, 
            Data_train=data_subsets['train'], 
            End_validation=end_times['validation'], 
            End_train=end_times['train'], 
            DatasetWithFeatures=dataset_withfeatures,
            Dataset_start_time=start_date_time, 
            Dataset_end_time=last_date_time,
            InitialLags=lags, 
            weight_para=if_use_weight,
            ExogenousFeatureType=ExogenousFeatureType, 
            FeatureSetReduction=FeatureSetReduction, 
            FeatureSetReductionMethod=FeatureSetReductionMethod, 
            bayesian_trial=20, 
            random_state_hyper=random_state_hyper,
            LastObservedDateTime=last_date_time, 
            Frequency=sf, 
            BacktestSteps=backtest_steps,
            PredictionSteps=forecasting_steps
        )

        list_of_results = []
        for model_name in list_of_models:
            result = ModelCreateTrainTestAndPredict(
                model_name=model_name,
                dataset=dataset,
                ...
                DatasetSelectedFeatures=dataset_feature_selection,
                DataMissing=missing,
                Weight=weight,
                Select_hyperparameters=True,
                Default_hyperparameters=None,
